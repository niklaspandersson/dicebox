...
function parseBackendHost() {
  ...
  if (match) {
    const isSecure = match[1].toLowerCase() === 'https';
    return { host: match[2], isSecure };
  }

  // No protocol specified - will infer from frontend
  return { host: BACKEND_HOST, isSecure: null };
}
...
export function getWebSocketUrl() {
  if (parsedBackend) {
    ...
    const isSecure = parsedBackend.isSecure ?? (window.location.protocol === 'https:');
