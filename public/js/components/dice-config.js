...
class DiceConfig extends HTMLElement {
  ...
  toggleColorDropdown(setId) {
    const dropdown = this.querySelector(
      `.color-dropdown[data-set-id="${setId}"]`,
    );
  ...
  closeAllDropdowns() {
    this.querySelectorAll(".color-dropdown.open").forEach((d) =>
  ...
  addDiceSet() {
    ...
    const availableColor =
      DICE_COLORS.find((c) => !usedColors.has(c.hex)) || DICE_COLORS[0];
  ...
  emitChange() {
    this.dispatchEvent(
      new CustomEvent("dice-config-changed", {
        bubbles: true,
