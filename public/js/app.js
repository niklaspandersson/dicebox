...
class DiceBoxApp {
  ...
  setupMessageHandlers() {
    this.messageRouter
      ...
      .onMessage(MSG.ROLL_REQUEST, (peerId, msg) => this.handleRollRequestMsg(peerId, msg));
  }
  ...
  setupEventListeners() {
    ...
    // Dice events
    // roll-requested: player initiates roll, values will be generated by a peer
    document.addEventListener('roll-requested', (e) => {
  ...
  handleDiceRollMsg(peerId, roll) {
    ...
    // Validate roll values and handle pending requests via RollManager
    if (!this.rollManager.handleDiceRoll(peerId, roll)) {
      console.warn('Received invalid roll, ignoring');
      return;
    }
  ...
  enterRoom() {
    ...
    // Initialize roll manager with current room context
    this.rollManager.init({
      messageRouter: this.messageRouter,
      meshState: this.roomManager.getMeshState(),
  ...
  /**
   * Handle roll request from local player
   * Delegates roll generation to a random peer for anti-cheat
   */
  async handleRollRequested({ diceSets, holders, lockedDice }) {
    const meshState = this.roomManager.getMeshState();
    const holdersMap = new Map(holders);
    ...
    try {
      ...
      // Complete the roll animation with received values
      if (this.diceRoller) {
        this.diceRoller.completeRoll(rollResults, lockedDice);
      }
    } catch (error) {
      console.error('Roll request failed:', error);
      // Cancel the roll animation
      if (this.diceRoller) {
  ...
  /**
   * Handle incoming roll request from another peer
   * If we're the designated generator, generate values and broadcast
   */
  handleRollRequestMsg(fromPeerId, message) {
    this.rollManager.handleRollRequest(fromPeerId, message);
  }
  ...
  leaveRoom() {
    ...
    // Clean up roll manager
    this.rollManager.cleanup();
