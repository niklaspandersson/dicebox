...
const TURN_CONFIG = {
  urls: process.env.TURN_URL
    ? process.env.TURN_URL.split(",").map((u) => u.trim())
    : null,
...
function isValidRoomId(roomId) {
  ...
  return (
    roomId.length >= 4 && roomId.length <= 32 && /^[a-zA-Z0-9-_]+$/.test(roomId)
  );
}
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", async (data) => {
    ...
    if (!peerId) {
      ...
      if (
        existingSession &&
        now - existingSession.lastSeen < SESSION_EXPIRY_SECONDS * 1000
      ) {
        ...
        logger.info(
          { peerId: truncatePeerId(peerId), restored: true },
          "Session restored",
        );
        ...
        await storage.setSession(sessionToken, {
          peerId,
          roomId: null,
          lastSeen: now,
        });
        ...
        logger.info(
          { peerId: truncatePeerId(peerId), restored: false },
          "New session created",
        );
    ...
    switch (message.type) {
      ...
      case "create-room": {
        ...
        if (!isValidRoomId(roomId)) {
          sendTo(ws, {
            type: "create-room-failed",
            roomId,
            reason: "Invalid room ID format",
          });
          return;
        }
      ...
      case "leave-room": {
        if (peer && peer.roomId) {
          ...
          logger.info(
            { roomId, peerId: truncatePeerId(peerId) },
            "Peer left room",
          );
  ...
  ws.on("error", (error) => {
    logger.error(
      { peerId: truncatePeerId(peerId), error: error.message },
      "WebSocket error",
    );
  });
});
